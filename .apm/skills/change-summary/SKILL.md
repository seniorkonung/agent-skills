---
name: change-summary
description: Produces a high-level, human-readable explanation of repository work performed by the current agent, connecting meaningful changes to their intent, resulting behavior, decisions, verification, and limitations. Use only when the user explicitly asks for a summary or explanation of that work; not automatically at completion, for routine progress updates, or for independent review of someone else's work.
---
...
of work performed by the current agent. Naming this skill directly counts as an
explicit request. The work may be complete or still in progress if the user asks
for an explanation of what has been done so far.

Do not invoke it automatically when implementation ends. Do not use it to:

- provide a routine conversational progress update;
- reconstruct historical changes or work performed by another agent;
- review correctness, conformance, or engineering quality;
- write release notes, a changelog, or a commit message; or
- replace detailed documentation the user explicitly requested.

Those jobs may use the same repository evidence, but they have different
audiences and standards.

## Establish the Work Being Explained

Default to repository work the current agent performed for the active user
request. Follow any broader or narrower boundary the user gives, such as the
whole session or one completed task. If multiple plausible boundaries would
materially change the account, state the ambiguity and ask for the smallest

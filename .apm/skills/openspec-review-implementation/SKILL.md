...
Judge engineering soundness, OpenSpec conformance, and ordinary code quality.
Keep the current result in `<change-root>/implementation-review.md`.
...
The active change provides context, not the scope delivered by the outgoing
...
Cross-check both directions: every included work item needs outgoing delivery or
...
still resolve to the recorded head and base. If either moved, rediscover and
review the new target.
...
- unrelated outgoing work remains explicit.
...
finding only when evidence disproves its failure mode or supplies a binding
constraint. Human acceptance changes disposition, not evidence.
...
current complete range without stale targets, resolved findings, duplicate
symptoms, or review history.
...
tracked work for later implementation. Never implement the finding. Keep it in
the report, hand the work items to the user, and re-audit only after a separate
Apply run is committed and the user requests another complete pre-push review.

## Handoff

Report the upstream and reviewed head, selected change, review units and work
...
- the immutable target contains every outgoing commit, no worktree changes, and
  no review report;
